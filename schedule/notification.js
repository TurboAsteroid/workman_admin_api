module.exports = function(app, config, firebase_admin) {
    ...
    async function createNotification (incidentGroup_id, row_id, user_id, group_id, incident_id, type, calendar_id = null) {
        try {
            console.warn("createNotification");
            ...
            const [rows, fields] = await connection.execute("insert into notification (incidentGroup_id, row_id, user_id, user_type, calendar_id) values (?,?,?,?,?)", [incidentGroup_id, row_id, user_id, type, calendar_id]);
            let payload = {
                data: {
    ...
    async function getNotificationList() {
        ...
        for (let i in rows2) {
            ...
            let [rows_x, fields_x] = await connection.execute(`select * from grouprows
                left join grouprowcalendars
                    on grouprowcalendars.row_id = grouprows.id
