...
module.exports = function (app, config, firebase_admin, router) {
  ...
  async function addincident (group_id, incident_id) {
    ...
    await DataBase.Execute('insert into incidentgroups (incident_id, group_id) values (?,?)', [incident_id, group_id])
    // connection.end();
  }
  ...
  router.post('/incident/notificationstatus', async function (req, res, next) {
    ...
    switch (req.body.status) {
      case 'checked':
        await DataBase.Execute('update notification SET complete = 1, timecheck = NOW() where id = ?', [req.body.notification_id])
        const [notR, notF] = await DataBase.Execute('select incidentgroups.id as incidentgroup_id from notification left join incidentgroups on incidentgroups.id = notification.incidentGroup_id where notification.id = ?', [req.body.notification_id])
        await DataBase.Execute('update incidentgroups SET complete = 1 where id = ?', [notR[0].incidentgroup_id])
        break
      case 'received':
        await DataBase.Execute('update notification SET timeget = NOW() where id = ?', [req.body.notification_id])
        break
      default:
  ...
  router.get('/incident/getbynotification', async function (req, res, next) {
    ...
    if (!notification_id) {
      return res.json({
        status: 'error',
        message: 'Не указан ID инцидента'
      })
    }

    // const connection = await DataBase.GetDB();// const connection = await mysql.createConnection(config.dbConfig);
    const [incR, incF] = await DataBase.Execute('select incident.*, DATE_FORMAT(incident.datetime, "%H:%i:%S %d-%m-%Y") as time from notification ' +
  ...
  router.post('/incident/new', async function (req, res, next) {
    // const connection = await DataBase.GetDB();// const connection = await mysql.createConnection(config.dbConfig);
    // const [rows2, fields2] = await DataBase.Execute('select * from Groups where id in ('+req.body.groups.join(',')+')', []);
    const [rows2, fields2] = await DataBase.Execute('insert into incident (title, description) values (?,?)', [req.body.title, req.body.description])
